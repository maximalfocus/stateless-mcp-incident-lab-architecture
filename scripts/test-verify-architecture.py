...
def main() -> None:
    ...
    mutations: list[tuple[str, Callable[[Path, Path], None]]] = [
        ...
        ("public-entry internal leak", lambda r, _p: replace(r / "scripts/verify-architecture.py", "return target == root + rule[\"allowed_entry\"]", "return True")),
        ("same-module import rejection", lambda r, _p: replace(r / "scripts/verify-architecture.py", "importer.startswith(root)", "False")),
        ...
        ("Accepted ADR reverted to Proposed", lambda r, _p: replace(r / "adr/0005-cloudfront-managed-https-origin-alb.md", "Status: Accepted", "Status: Proposed")),
        ...
        ("README pin for the accepted ADR", lambda r, _p: replace(r / "README.md", "`INFRA-004`, `INFRA-010`", "`INFRA-004`")),
        ...
        ("missing Supersedes relation", lambda r, _p: replace(r / "adr/0006-trusted-viewer-ip-waf-key.md", "- Supersedes viewer-IP mechanism in:", "- Related decision:")),
        ("wrong Supersedes target", lambda r, _p: replace(r / "adr/0006-trusted-viewer-ip-waf-key.md", "(0005-cloudfront-managed-https-origin-alb.md)", "(0004-ephemeral-unauthenticated-core-lab.md)")),
